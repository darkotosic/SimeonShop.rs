...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const staticRoutes = ['', '/products', '/about', '/contact', '/shipping', '/returns', '/size-guide', '/privacy-policy', '/terms-and-conditions'].map((path) => ({ url: `${siteUrl}${path}`, lastModified: new Date() }));
  try {
