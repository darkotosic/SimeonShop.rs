...
export function AdminSettingsPanel({ token }: { token: string }) {
  const [settings, setSettings] = useState<Record<string, StoreSetting>>({});
  const load = async () => { const rows = await getAdminSettings(token); setSettings(Object.fromEntries(rows.map((row) => [row.key, row]))); };
  useEffect(() => { load().catch(console.error); }, []);
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = new FormData(event.currentTarget); await Promise.all(keys.map((key) => updateAdminSetting(token, key, { value: String(form.get(key) || ''), value_type: 'string', is_public: true }))); await load(); }
